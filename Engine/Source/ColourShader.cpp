...
ColourShader::ColourShader(const ColourShader& other)
{
	...
	m_pMatrixBuffer = other.m_pMatrixBuffer;
}
...
bool ColourShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"Resources/Shaders/Colourvs.hlsl", L"Resources/Shaders/Colourps.hlsl");
	if (!result)
	{
		return false;
	}
...
bool ColourShader::InitializeShader(ID3D11Device* device, HWND hwnd, LPCWSTR vsFilename, LPCWSTR psFilename)
{
	HRESULT result;
...
void ColourShader::OutputShaderErrorMessage(ID3DBlob* errorMessage, HWND hwnd, LPCWSTR shaderFilename)
{
	char* compileErrors;
