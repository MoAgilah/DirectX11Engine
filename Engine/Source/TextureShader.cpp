...
bool TextureShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"Resources/Shaders/texturevs.hlsl", L"Resources/Shaders/textureps.hlsl");
	if (!result)
	{
		return false;
	}
...
bool TextureShader::InitializeShader(ID3D11Device* device, HWND hwnd, LPCWSTR vsFilename, LPCWSTR psFilename)
{
	HRESULT result;
...
void TextureShader::OutputShaderErrorMessage(ID3DBlob* errorMessage, HWND hwnd, LPCWSTR shaderFilename)
{
	char* compileErrors;
